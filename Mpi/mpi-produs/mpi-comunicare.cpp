...
void mpi_bcast_matrice_csr(MatriceCSR &Matrice, int rank) {
	...
	MPI_Bcast(&(Matrice.V_COL_INDEX_size), 1, MPI_LONG_LONG, 0, MPI_COMM_WORLD);
	MPI_Bcast(&(Matrice.ROW_INDEX_size), 1, MPI_LONG_LONG, 0, MPI_COMM_WORLD);

	if (rank != 0) {
		Matrice.V.resize(Matrice.V_COL_INDEX_size);
		Matrice.COL_INDEX.resize(Matrice.V_COL_INDEX_size);
		Matrice.ROW_INDEX.resize(Matrice.ROW_INDEX_size);
	}

	MPI_Bcast(&(Matrice.V[0]), Matrice.V_COL_INDEX_size, MPI_LONG_LONG, 0, MPI_COMM_WORLD);
	MPI_Bcast(&(Matrice.COL_INDEX[0]), Matrice.V_COL_INDEX_size, MPI_LONG_LONG, 0, MPI_COMM_WORLD);
	MPI_Bcast(&(Matrice.ROW_INDEX[0]), Matrice.ROW_INDEX_size, MPI_LONG_LONG, 0, MPI_COMM_WORLD);

	/*if (rank != 0) {
		std::cout << "RANK** " << rank << std::endl;

		for (int i = 0; i < Matrice.V_COL_INDEX_size; i++)
